...
import { CONDENSED, drawHiResLogo, drawPhotoAndScrims, metricText } from "./shared";

const KEYS = ["distance", "pace", "duration"] as const;

/**
 * Modelo "14" do usuário: tudo centralizado — rótulo pequeno na cor do
...
export const rotulosCentro: StoryLayout = {
  ...
  draw(ctx, data) {
    ...
    R.rows.forEach((row, i) => {
      ...
      textWithShadow(ctx, value, R.center, row.value.y, {
        font: valueFont(fitFontSize(ctx, value, valueFont, R.maxW, row.value.size, 60)),
        align: "center",
        shadowBlur: 12,
        shadowColor: "rgba(0,0,0,0.5)",
      });
